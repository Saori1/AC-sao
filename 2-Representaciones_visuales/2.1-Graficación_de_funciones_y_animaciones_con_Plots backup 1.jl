...
# ╔═╡ 33630c60-7dc8-4565-9524-075512b43508
@bind d Slider(0.05:0.05:3, default=0.5)

# ╔═╡ c4f9306f-25b9-4acd-a777-666ebfcacf3b
d

# ╔═╡ d271d2f9-ea57-4f65-8d85-6a24ec7c3ef8
begin
	plot(sin, -π:d:π, title = " Funciones trigonométricas ",  color = "darkorange", label = "\$\\sin(x)\$", marker = true)
	plot!(cos, -π:d:π, title = " Funciones trigonométricas ",  color = "darkblue", label = "\$\\cos(x)\$", marker = true)
end
...
quiver((-2:0.2:2)', (-2:0.2:2) , quiver=g, color="purple") #¡La apóstrofe ' traspone matrices!
...
# ╔═╡ 48220590-9d1c-4b86-896c-6fc7bf0c01de
begin

	H, θ, r = 6, 8, 2 #Definimos los valores de los parámetros H, θ, r


	x(t)= r*cos(θ)*t   #así como la función x
    y(t)= ((H+r)*sin(θ)*t)-((9.81*(t^2))/2)  #y la función y
	
	@gif for t in 10:1:10 #Por cada t en 10:1:10
	scatter(y.(range(-10,10,step=1)),      #grafica r*cos(θ)*t y ((H+r)*sin(θ)*t)-((9.81*(t^2))/2) para todas las x y y en
		 legend = false, title = "Tiro parabólico") #el rango de -10 a 10 con tamaño de paso 1.
	end
end

# ╔═╡ cc3aee0f-4b71-45c5-bc78-41fd863e0832
begin
 x = 6
	y =7 #Definimos los valores de los parámetros A, B y C,
	i(x,y) = cos(x) + sin(y)    #así como la función f.
	
	@gif for t in 0:0.05:2π #Por cada t en 0:0.1:2π,
	plot(f.( range(0,2π, step = 0.05) ),      #grafica Asin(B(x-t/B))+C para todas las x en
		 legend = false, title = L"superficie") #el rango de 0 a 2π con tamaño de paso 0.1.
	end
end
